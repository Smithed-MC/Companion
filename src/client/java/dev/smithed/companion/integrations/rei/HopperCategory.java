...
public class HopperCategory<T extends Display> extends DisplayCategoryExtender<T> {

    public HopperCategory(Identifier id, Text title, ItemStack icon, BackgroundContainer background) {
        super(id, title, icon, background);
    }

    @Override
    public List<Widget> setupDisplay(T display, Rectangle bounds) {
        final Point startPoint = new Point(bounds.getX()+38, bounds.getY()+7);
