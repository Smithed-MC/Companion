...
public class ChestCategory<T extends Display> extends DisplayCategoryExtender<T> {

    public ChestCategory(Identifier id, Text title, ItemStack icon, BackgroundContainer background) {
        super(id, title, icon, background);
    }

    @Override
    public List<Widget> setupDisplay(T display, Rectangle bounds) {
        final Point startPoint = new Point(bounds.getX()+7, bounds.getY()+7);
        ...
        for (EntryIngredient ingredient: display.getInputEntries()) {
            widgets.add(Widgets.createSlot(new Point(startPoint.x + (18 * (i%9)), startPoint.y + (18 * (i/9))))
                    .entries(ingredient)
                    .markInput()
            );
            i += 1;
